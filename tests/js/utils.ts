...
export function testArrowTablesEqual(
  ...
  expect(table1.schema.fields.length).toStrictEqual(table2.schema.fields.length);
  ...
  for (const fieldName of fieldNames) {
    ...
    expect(
      vector1.toJSON(),
      `data arrays should be equal for column ${fieldName}`).toStrictEqual(
      vector2.toJSON(),
    );
  }
}
...
/** Test that two Arrow tables are equal */
export function testArrowTablesEqualOld(
  t: Test,
  table1: Table,
  table2: Table
): void {
